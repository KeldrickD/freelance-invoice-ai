...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      ...
      for (let i = 0; i < Number(nextId); i++) {
        try {
          // Fetch invoice details
          const inv = await (window as any).ethereum.request({
            method: 'eth_call',
            params: [{
              to: CONTRACT_ADDRESS,
              data: '0x' // TODO: ABI encode getInvoice(i)
            }, 'latest']
          });
          
          // For now, use demo data but structure it properly
          // In production, decode the actual contract response
          invs.push({
            id: i,
            freelancer: '0x1234...5678',
            client: '0x8765...4321',
            totalAmount: 100 * 10**6n, // 1000 USDC with 6 decimals
            paid: false,
            projectDescription: `Project ${i + 1}`,
            milestoneAmounts: [300 * 10**6n, 400 * 10**6n, 300 * 10**6n],
  ...
  const handleComplete = async (invoiceId: number, milestoneIndex: number) => {
    try {
      ...
      if (primaryWallet) {
        // Use Smart Wallet gasless transaction
        await primaryWallet.gasless({
          address: CONTRACT_ADDRESS,
          abi: [
            {
              inputs: [
        ...
        toast.success('Milestone completed gaslessly! 🚀');
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50indigo-100">
      <div className="max-w-6auto p-6">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4ont-bold text-gray-900 Invoice Dashboard</h1>
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            View and manage your freelance invoices with Smart Wallet technology
          </p>
          {primaryWallet && (
            <div className="mt-4 inline-flex items-center px-4 py-2 bg-green-100-green-800 rounded-full text-sm font-medium">             <span className="mr-2">✨</span>
        ...
        {!address ? (
          <div className="text-center py-16">
            <div className="bg-white rounded-2 shadow-xl p-8ax-w-md mx-auto">
              <h2 className="text-2font-semibold text-gray-900mb-6">Connect Your Wallet</h2>
              <div className="space-y-4">
                ...
                <div>
                  <DynamicConnectButton 
                    buttonClassName="w-full bg-gradient-to-r from-blue-600o-purple-600over:from-blue-700over:to-purple-70-white font-medium py-3 rounded-lg transition-all transform hover:scale-105"
                  >
          ...
          <>
            {/* Stats */}
            <div className="bg-white rounded-2l shadow-xl p-6 mb-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center">
                  <p className="text-sm text-gray-60">Total Invoices</p>
                  <p className="text-3ont-bold text-blue-600>{nextId ? Number(nextId) : 0}</p>
                </div>
                <div className="text-center">
                ...
                <div className="text-center">
                  <p className="text-sm text-gray-60">Wallet Type</p>
            ...
            {loading && (
              <div className="text-center py-8">
                <div className="inline-flex items-center px-42blue-100t-blue-80">
                  <div className="animate-spin rounded-full h-4 w-4 border-b-2border-blue-600">
                  Loading...
                </div>
              </div>
            )}
            ...
            <div className="space-y-6">
              {invoices.map(inv => (
                <div key={inv.id} className="bg-white rounded-2l shadow-xl p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="text-xl font-bold text-gray-900">Invoice #{inv.id}</h3>
                      <p className="text-gray-600projectDescription}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-2nt-bold text-green-600>{formatUnits(inv.totalAmount, 6)} USDC</p>
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                        inv.paid ? 'bg-green-10 text-green-800' : 'bg-yellow-10 text-yellow-800'
                      }`}>
                  ...
                  <div>
                    <h4 className="text-lg font-semibold text-gray-900mb-4">Milestones</h4>
                      ...
                      {inv.milestoneAmounts.map((amt: bigint, idx: number) => (
                        <div key={idx} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                          ...
                          <div className="flex items-center space-x-3">
                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                              inv.milestoneCompleted[idx] ? 'bg-green-10 text-green-800' : 'bg-yellow-10 text-yellow-800'
                            }`}>
                            ...
                            {!inv.milestoneCompleted[idx] && (
                              <>
                                <button 
                                  ...
                                  disabled={loading}
                                  className="px-4 py-2 bg-green-60hover:bg-green-700 disabled:bg-gray-40text-white text-sm font-medium rounded-lg transition-colors disabled:cursor-not-allowed"
                                >
                                ...
                                <button 
                                  ...
                                  disabled={loading}
                                  className="px-4py-2 bg-purple-600over:bg-purple-700 disabled:bg-gray-40text-white text-sm font-medium rounded-lg transition-colors disabled:cursor-not-allowed"
                                >
            ...
            {/* Features */}
            <div className="mt-12 bg-white rounded-2l shadow-xl p-8">
              <h3 className="text-2ont-bold text-gray-900 mb-6">💡 Dashboard Features</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center">
                    <span className="mr-3text-blue-600">✨</span>
